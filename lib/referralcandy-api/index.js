...
function ReferralCandy(config) {
  ...
  if (!['v1', 'v2'].includes(config.apiVersion)) {
  ...
  this.baseUrl = `https://my.referralcandy.com/api/${config.apiVersion}`
...
ReferralCandy.prototype.request = async function(method, key, data) {
  ...
  if (method === 'GET') {
    fullPath.search = new URLSearchParams(fullData).toString()
  }

  const res = await fetch(fullPath.href, {
    method,
    ...(method === 'POST' ? { body: signedData } : {}),
    headers: {
